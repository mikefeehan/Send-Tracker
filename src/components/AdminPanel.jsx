      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-sm">
          ...
          <input
            ...
            placeholder="PIN"
            autoFocus
            className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white text-center text-2xl tracking-[0.5em] placeholder-slate-600 focus:outline-none focus:border-pink-500 mb-4"
            onKeyDown={e => { if (e.key === 'Enter' && pin === ADMIN_PIN) setAuthed(true) }}
          />
          <button
            onClick={() => pin === ADMIN_PIN ? setAuthed(true) : setMsg('❌ Wrong PIN')}
            className="w-full py-3 rounded-xl font-bold text-white bg-gradient-to-r from-pink-600 to-purple-600 active:scale-95 transition-all"
          >
            Enter
          </button>
    ...
    <div className="min-h-screen bg-slate-950 p-4 pb-20">
      <div className="max-w-lg mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <a href="/" className="text-sm text-pink-400 font-semibold">← Back to App</a>
        </div>
        ...
        {busy && (
          <div className="text-center text-pink-400 text-sm font-semibold animate-pulse">{busy}</div>
        )}
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-4">
          ...
          <div className="space-y-3">
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-pink-500"
            />
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-pink-500"
                />
              </div>
              <div>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl font-bold text-white bg-gradient-to-r from-pink-600 to-purple-600 disabled:opacity-40 active:scale-95 transition-all"
            >
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-4">
          ...
          <textarea
            ...
            rows={3}
            className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-pink-500 resize-none mb-3"
          />
          <button
              ...
              <div key={u.userId}>
                ...
                <div
                  ...
                  className={`flex items-center justify-between rounded-xl px-4 py-3 cursor-pointer transition-all ${
                    expandedUser === u.userId ? 'bg-slate-700 border border-pink-500/30' : 'bg-slate-800 hover:bg-slate-750'
                  }`}
                >
                ...
                {expandedUser === u.userId && (
                  <div className="mt-1 ml-4 space-y-1.5 border-l-2 border-pink-500/20 pl-3 py-2">
                    {u.drinks.map(d => (
                      <div key={d.id} className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm text-white font-medium">
                            ...
                            <span className="text-pink-400 ml-1">{d.points}pts</span>
                          </div>
